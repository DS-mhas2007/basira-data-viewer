...
export function ReportDocument({ data }: { data: ReportData }) {
  ...
  const withQuality = variant === "brief" || variant === "full";
  const withInsights = variant !== "brief";
  const compact = variant === "charts";
  const withClosing = variant === "full";
  const insights = withInsights ? data.insights.slice(0, 5) : [];
  const total = 1 + (withQuality ? 1 : 0) + insights.length + (withClosing ? 1 : 0);
  const h = data.health;
      ...
      <Page index={++page} total={total} footer={false}>
        ...
        >
          ...
          <div style={{ fontSize: 17, lineHeight: 2.1, color: MUTED }}>
            ...
            <p style={{ margin: 0 }}>
              {data.autoGenerated ? "عدد الاستنتاجات المولّدة تلقائياً: " : "عدد الاستنتاجات المثبتة: "}
              <span style={{ color: TEXT }}>{num(insights.length)}</span>
            </p>
          </div>
