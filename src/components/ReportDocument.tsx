...
import { cleanCell, dirOf, formatSql, tokenizeSql } from "@/lib/report-format";
...
export interface ReportData {
  ...
  /** ختم المصداقية الرقمي (بصمة SHA-256 محلية). */
  seal?: { shortHash: string; issuedAt: string } | null;
}

const SQL_COLOR = { kw: VIOLET, str: "#F5C978", num: TEAL, plain: TEXT } as const;

/** كود SQL مُجمَّل ومُلوَّن بخط أحادي المسافة، باتجاه LTR ثابت. */
      ...
      {lines.map((line, i) => (
        <div key={i}>
          {tokenizeSql(line).map((t, j) => (
            <span
              key={j}
              style={{ color: SQL_COLOR[t.kind], fontWeight: t.kind === "kw" ? 700 : 400 }}
            >
              {t.text}
            </span>
          ))}
...
function Kpis({ items, big = false }: { items: ReturnType<typeof executiveKpis>; big?: boolean }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: big || list.length <= 3 ? "1fr" : "1fr 1fr", gap: big ? 16 : 12 }}>
      {list.map((k, i) => (
        <Card key={i} style={{ padding: big ? 26 : 18 }}>
          <p style={{ margin: 0, fontSize: big ? 15 : 13, color: MUTED }}>{k.label}</p>
          <p
            dir="auto"
            style={{
              margin: "8px 0 0",
          ...
          <p dir="auto" style={{ margin: "8px 0 0", fontSize: big ? 13 : 11.5, color: MUTED }}>
            {k.hint}
          </p>
        </Card>
      ))}
          ...
          {data.seal && (
            <div
              style={{
                ...
                borderRadius: 14,
                padding: "12px 18px",
                background: "rgba(96,245,210,0.06)",
              }}
            >
              <p style={{ margin: 0, fontSize: 13, color: TEAL, fontWeight: 700 }}>
                ✅ ختم المصداقية — تحليل محلي موثّق
              </p>
              <p dir="ltr" style={{ margin: 0, fontSize: 11, color: MUTED, fontFamily: "monospace" }}>
                SHA-256: {data.seal.shortHash}…
              </p>
              <p style={{ margin: 0, fontSize: 11, color: MUTED }}>
                {new Date(data.seal.issuedAt).toLocaleString("ar-EG")}
              </p>
            </div>
          )}
        </div>
